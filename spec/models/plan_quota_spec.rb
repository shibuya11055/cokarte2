...
RSpec.describe 'プラン別クォータ' do
  let(:free_user)  { create(:user, plan_tier: 'free',  clients_count: 0) }
  let(:basic_user) { create(:user, plan_tier: 'basic', clients_count: 0) }
  let(:pro_user)   { create(:user, plan_tier: 'pro',   clients_count: 0) }
