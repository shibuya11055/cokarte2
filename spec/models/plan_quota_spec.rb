...
RSpec.describe 'プラン別クォータ' do
  let(:free_user)  { User.create!(first_name: 'Free',  last_name: 'User', email: 'free@example.com',  password: 'Password1!', confirmed_at: Time.current, plan_tier: 'free',  clients_count: 0) }
  let(:basic_user) { User.create!(first_name: 'Basic', last_name: 'User', email: 'basic@example.com', password: 'Password1!', confirmed_at: Time.current, plan_tier: 'basic', clients_count: 0) }
  let(:pro_user)   { User.create!(first_name: 'Pro',   last_name: 'User', email: 'pro@example.com',   password: 'Password1!', confirmed_at: Time.current, plan_tier: 'pro',   clients_count: 0) }
